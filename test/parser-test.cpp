...
TEST_CASE("Parser parses chunks correctly", "[parser]") {
    ...
    CHECK(chunk1.commands[0].type == xrf::CommandType::Input);
    CHECK(chunk1.commands[1].type == xrf::CommandType::Output);
    CHECK(chunk1.commands[2].type == xrf::CommandType::Pop);
    CHECK(chunk1.commands[3].type == xrf::CommandType::Dup);
    CHECK(chunk1.commands[4].type == xrf::CommandType::Swap);

    auto chunk2 = chunks[1];
    CHECK(chunk2.line == 1);
    CHECK(chunk2.col == 7);
    REQUIRE(chunk2.commands.size() == 5);
    CHECK(chunk2.commands[0].type == xrf::CommandType::Inc);
    CHECK(chunk2.commands[1].type == xrf::CommandType::Dec);
    CHECK(chunk2.commands[2].type == xrf::CommandType::Add);
    CHECK(chunk2.commands[3].type == xrf::CommandType::IgnoreFirst);
    CHECK(chunk2.commands[4].type == xrf::CommandType::Bottom);

    auto chunk3 = chunks[2];
    CHECK(chunk3.line == 3);
    CHECK(chunk3.col == 2);
    REQUIRE(chunk3.commands.size() == 5);
    CHECK(chunk3.commands[0].type == xrf::CommandType::Jump);
    CHECK(chunk3.commands[1].type == xrf::CommandType::Exit);
    CHECK(chunk3.commands[2].type == xrf::CommandType::IgnoreVisited);
    CHECK(chunk3.commands[3].type == xrf::CommandType::Randomize);
    CHECK(chunk3.commands[4].type == xrf::CommandType::Sub);

    auto chunk4 = chunks[3];
    CHECK(chunk4.line == 3);
    CHECK(chunk4.col == 8);
    REQUIRE(chunk4.commands.size() == 5);
    CHECK(chunk4.commands[0].type == xrf::CommandType::Nop);
    CHECK(chunk4.commands[1].type == xrf::CommandType::Nop);
    CHECK(chunk4.commands[2].type == xrf::CommandType::Nop);
    CHECK(chunk4.commands[3].type == xrf::CommandType::Nop);
    CHECK(chunk4.commands[4].type == xrf::CommandType::Nop);
}
